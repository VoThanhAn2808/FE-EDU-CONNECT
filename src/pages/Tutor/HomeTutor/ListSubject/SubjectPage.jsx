...
function SubjectPage() {
    ...
    useEffect(() => {
        axios
            .get(`http://capstone.recoff.cloud:8081/educonnect/tutor/course?tutorid=${userId}`)
            .then((response) => {
                ...
                {data.map((item) => (
                    <Grid item xs={12} sm={6} md={5} lg={3} key={item.classcourseid}>
                        <Box className='container'>
                            <img src={`http://capstone.recoff.cloud:8081/edu/file/files/` + item.img} alt={item.courseName} className="subject-img" />
