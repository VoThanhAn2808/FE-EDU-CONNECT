...
function TeacherProgramList() {
    ...
    useEffect(() => {
        axios
            .get(`http://capstone.recoff.cloud:8081/exersice/findexersice?bookid=${bookid}`)
            .then((response) => {
    ...
    useEffect(() => {
        axios
            .get(`http://capstone.recoff.cloud:8081/exersice/fileexercise?bookid=${bookid}`)
            .then((response) => {
    ...
    useEffect(() => {
        axios
            .get(`http://capstone.recoff.cloud:8081/exersice/videoexercise?bookid=${bookid}`)
            .then((response) => {
    ...
    useEffect(() => {
        axios
            .get(`http://capstone.recoff.cloud:8081/exersice/classroomexercise?bookid=${bookid}`)
            .then((response) => {
    ...
    useEffect(() => {
        axios
            .get(`http://capstone.recoff.cloud:8081/exersice/homeworkexercise?bookid=${bookid}`)
            .then((response) => {
    ...
    const handleLinkClick = async (fileid, event, file) => {
        ...
        try {
            await axios.put(
                `http://capstone.recoff.cloud:8081/exersice/fileexercise/file/${fileid}`,
                {},
                {
                    headers: {
    ...
    const handleVideoClick = async (fileid, event, file) => {
        ...
        try {
            await axios.put(
                `http://capstone.recoff.cloud:8081/exersice/videoexercise/video/${fileid}`,
                {},
                {
                    headers: {
                        "Content-Type": "application/json",
                            ...
                            if (files.exerciseid === item.exerciseid) {
                                return (
                                    <Box key={keyf}>
                                        {files.status === 2 ? (
                                            <>
                                                <Links href={`http://capstone.recoff.cloud:8081/edu/file/fileuser/${files.files}/${data.tutorid}`} target="_blank" sx={{ textDecoration: 'none', color: 'black' }} onClick={(event) => handleLinkClick(files.fileid, event, files.files)}>
                                            ...
                                            <>
                                                <Links href={`http://capstone.recoff.cloud:8081/edu/file/fileuser/${files.files}/${data.tutorid}`} target="_blank" sx={{ textDecoration: 'none', color: 'black' }}>
                                                    <Box sx={{ display: "flex", alignItems: "center" }}>
                                                        <InsertDriveFileIcon sx={{ fontSize: "25px", marginLeft: "4%" }} />
                                                        <Typography sx={{ fontSize: "25px", marginLeft: "1%" }}> {files.namefile}</Typography>
