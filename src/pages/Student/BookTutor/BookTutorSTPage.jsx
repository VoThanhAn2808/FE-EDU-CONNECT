...
function BookTutorSTPage() {
    ...
    useEffect(() => {
        axios
            .get(`http://capstone.recoff.cloud:8081/educonnect/tutor/booktutor?tutorid=${tutorid}&classcourseid=${classcourseid}`)
            .then((response) => {
    ...
    useEffect(() => {
        axios
            .get(`http://capstone.recoff.cloud:8081/tutorByCourse/find4TutorByCourse?CourseId=${classcourseid}`)
            .then((response) => {
    ...
    useEffect(() => {
        axios
            .get(`http://capstone.recoff.cloud:8081/course/findCourseByTutor?tutorid=${tutorid}`)
            .then((response) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            await axios.post(
                "http://capstone.recoff.cloud:8081/book/bookcourse",
                {
                    studentId: studentid,
                    tutorId: tutorid,
    ...
    const handleSubmitTry = async (event) => {
        ...
        try {
            await axios.post(
                "http://capstone.recoff.cloud:8081/trylearn/booktrylearn",
                {
                    studentid: studentid,
                    tutorid: tutorid,
                    classcourseid: classcourseid,
                ...
                <Grid container spacing={1}>
                    <Grid item xs={5} >
                        <Box className="tutor-infor">
                            <img src={`http://capstone.recoff.cloud:8081/edu/file/fileuser/${data.img}/${data.tutorId}`} alt={data.fullname} className="tutor-img" />
                        </Box>
                    </Grid>
                    <Grid item xs={7}>
                    ...
                    {course.map((item, index) => (
                        <Grid item xs={3} key={index}>
                            <Box className='top4couse' style={{ position: 'relative' }}>
                                ...
                                <img src={`http://capstone.recoff.cloud:8081/edu/file/files/` + item.img}
                                    alt={item.courseName}
                    ...
                    {page.map((item, index) => (
                        <Grid item xs={3} key={index}>
                            <Box className='top4couse'>
                                ...
                                <img src={`http://capstone.recoff.cloud:8081/edu/file/fileuser/${item.img}/${item.tutorid}`}
                                    style={{ width: '120px', height: '170px' }}
