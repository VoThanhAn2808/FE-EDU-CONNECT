...
function TutorRegisterManagement() {
    ...
    const handleSend = async (event) => {
        ...
        try {
            const response = await axios.put(
                `http://capstone.recoff.cloud:8081/staffsconnect/accepttutor`,
                {
                    tutorid: tutor,
    ...
    const handleSendfinal = async (event) => {
        ...
        try {
            const response = await axios.put(
                `http://capstone.recoff.cloud:8081/staffsconnect/accepttutorfinal`,
                {
                    tutorid: tutor,
                    staffid: decodedToken.id,
                    message: message,
    ...
    const handleDelete = async (event, tutorid) => {
        ...
        try {
            const response = await axios.delete(
                `http://capstone.recoff.cloud:8081/staffsconnect/deletetutorregister/${tutorid}`);
            showSnackbar(response.data);
            window.location.reload();
        } catch (error) {
            console.error(error);
    ...
    useEffect(() => {
        axios
            .get(`http://capstone.recoff.cloud:8081/staffsconnect/listwaitforconfirm?page=${page}`)
            .then((response) => {
        ...
        axios
            .get(`http://capstone.recoff.cloud:8081/staffsconnect/pageforwaittutor`)
            .then((response) => {
                                        ...
                                        return (
                                            <TableRow key={item.tutorid}>
                                                ...
                                                <TableCell sx={{ fontSize: "15px", fontFamily: "cursive", textAlign: "center" }}>{item.cv ? (
                                                    <Link style={{ textDecoration: "none" }} href={`http://capstone.recoff.cloud:8081/edu/file/fileuser/${item.cv}/${item.tutorid}`} target="_blank" rel="noopener noreferrer">
                                                        Tải File
                                                    </Link>
                                                ) : (
                                                    "No file available"
