...
function DiscountManagement() {
    ...
    useEffect(() => {
        axios.post('http://capstone.recoff.cloud:8081/discount/listdiscount', dataToSend)
            .then((response) => {
    ...
    const onDeleteRow = async (id) => {
        try {
            const listDelete = [id];
            const response = await axios.post('http://capstone.recoff.cloud:8081/discount/deleteDiscount', listDelete);
                                ...
                                {Array.isArray(dataDicount.listDiscount) ? (
                                    (dataDicount.listDiscount).map((item, index) => (
                                        <TableRow key={item.discountid}>
                                            ...
                                            <TableCell onClick={() => handleCellClick(item.discountid)} style={{ fontSize: "10px", fontFamily: "cursive", textAlign: "center", width: '18px' }}><img src={'http://capstone.recoff.cloud:8081/edu/file/files/' + item.img} alt={`Discount Image for ${item.title}`} style={{ maxWidth: '100%', maxHeight: '100%', }} /></TableCell>
