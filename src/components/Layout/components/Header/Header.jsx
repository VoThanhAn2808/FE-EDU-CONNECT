...
function Header() {
    ...
    const handleClickPay = async (event, tutorid) => {
        ...
        try {
            await axios.post(
                `http://capstone.recoff.cloud:8081/educonnect/paymenttutor`,
                {
                    tutorid: tutorid,
                    money: money,
        ...
        try {
            if (role === 1) {
                const check = await axios.get(
                    `http://capstone.recoff.cloud:8081/student/checkstudent?studentid=${decodedTokenRef.current.id}`,
                );
                setCheckProfile(check.data);
            }
            if (role === 2) {
                const check = await axios.get(
                    `http://capstone.recoff.cloud:8081/educonnect/checktutor?tutorid=${decodedTokenRef.current.id}`,
                );
                setCheckProfile(check.data);
            }
        } catch (error) {
    ...
    useEffect(() => {
        try {
            if (token !== null) {
                ...
                if (role === 1) {
                    axios
                        .get(`http://capstone.recoff.cloud:8081/student/viewstudent?email=${decodedTokenRef.current.id}`)
                        .then((response) => {
                    ...
                    axios
                        .get(`http://capstone.recoff.cloud:8081/educonnect/viewTutor?tutorId=${decodedTokenRef.current.id}`)
                        .then((response) => {
                            setData(response.data);
                        })
                        .catch((error) => {
                    ...
                    axios
                        .get(`http://capstone.recoff.cloud:8081/educonnect/showbank?tutorid=${decodedTokenRef.current.id}`)
                        .then((response) => {
                    ...
                    axios
                        .get(`http://capstone.recoff.cloud:8081/educonnect/historypay?tutorid=${decodedTokenRef.current.id}`)
                        .then((response) => {
                    ...
                    axios
                        .get(`http://capstone.recoff.cloud:8081/staffsconnect/ViewInfoStaff?staffId=${decodedTokenRef.current.id}`)
                        .then((response) => {
                            setData(response.data);
                        })
                        .catch((error) => {
                                    ...
                                    >
                                        <IconButton>
                                            {data.img && decodedTokenRef.current ? (
                                                <Avatar
                                                    alt={data.fullname}
                                                    src={`http://capstone.recoff.cloud:8081/edu/file/fileuser/${data.img}/${decodedTokenRef.current.id}`}
                                                    sx={{
                                                        height: "55px",
                                                        width: "55px",
                                                    }}
