...
export default function UpdateModal(props) {
  ...
  useEffect(() => {
    axios.post('http://capstone.recoff.cloud:8081/discount/detailDiscount', dataToSend)
      .then((response) => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://capstone.recoff.cloud:8081/edu/file/files/${filename}`, {
  ...
  const handleSubmit = async (event) => {
    ...
    if (grdImageDisplayStyle === "none") { // input image hide
      if (image == null) {
        ...
        try {
          //Upload image
          const responseUploadImage = await axios.post('http://capstone.recoff.cloud:8081/edu/file/upload/discount', formData, {
            headers: {
          ...
          if (responseUploadImage.data.message === -1) {
            ...
            const response = await axios.put(`http://capstone.recoff.cloud:8081/staffsconnect/discount/updateDiscount/${decodedToken.id}`, myObject);
          ...
          await axios.delete(`http://capstone.recoff.cloud:8081/edu/file/deleteFile/discount/${image.name}`);
        }
      }
    } else {
      const checkstartdate = startDate ? startDate : dataDetailDiscount.startDate
