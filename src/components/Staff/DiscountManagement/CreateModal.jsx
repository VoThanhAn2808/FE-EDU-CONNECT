...
export default function CreateModal(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      //Upload image
      const responseUploadImage = await axios.post('http://capstone.recoff.cloud:8081/edu/file/uploadImages', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      if (responseUploadImage.status === 200) {
        const response = await axios.post('http://capstone.recoff.cloud:8081/discount/adddiscount', myObject);
